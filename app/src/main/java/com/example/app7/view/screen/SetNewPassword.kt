...
@Composable
fun SetNewPassword(controller: NavHostController) {
    ...
    email = prefController.setUserEmail()
    ...
    Column(modifier = Modifier.background(color = backColor)) {
        ...
        Column(
            ...
            if(passwordCheck) {
                Text(
                    "Set different password than previous one",
                    modifier = Modifier
                        .fillMaxWidth(),
                    color = errorColor,
                    textAlign = TextAlign.Center
            ...
            Button(
                onClick = {
                    ...
                    if (email.isNotEmpty() && password.isNotEmpty()) {
                        val oldPassword = homeController.getOldPassword(email)
                        if(oldPassword != password) {
                            homeController.setNewPassword(email, password)
                                controller.navigate(ScreenRoute.Login.route) {
                                    popUpTo(ScreenRoute.SetNewPassword.route) {
                                        inclusive = true
                                    }
                            }
                        } else {
                ...
                Text(text = "Set new Password", fontSize = 20.sp)
            }
        }
